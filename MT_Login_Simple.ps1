# A6-9V MetaTrader Login Information Script
param(
...
if ($MT4_Process) {
    Write-Host "[SUCCESS] MT4 Terminal detected (PID: $($MT4_Process.Id))" -ForegroundColor Green
} else {
    Write-Host "[WARNING] MT4 Terminal not running" -ForegroundColor Red
}

# Check MT5  
$MT5_Process = Get-Process -Name "terminal64" -ErrorAction SilentlyContinue
if ($MT5_Process) {
    Write-Host "[SUCCESS] MT5 Terminal detected (PID: $($MT5_Process.Id))" -ForegroundColor Green
} else {
    Write-Host "[WARNING] MT5 Terminal not running" -ForegroundColor Red
}

Write-Host ""
Write-Host "TRADING ACCOUNT CREDENTIALS:" -ForegroundColor Yellow
...
Write-Host "MANUAL LOGIN REQUIRED:" -ForegroundColor Yellow
...
Write-Host ""
Write-Host "TRADING CHECKLIST:" -ForegroundColor Green
...
Write-Host ""
Write-Host "Organization: A6-9V | System Status: READY" -ForegroundColor Green
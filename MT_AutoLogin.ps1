...
param(
    [string]$Platform = "both",  # Options: mt4, mt5, both
    [int]$WaitTime = 30         # Wait time for platforms to load
)
...
function Login-MetaTrader {
    ...
    $process = Get-Process -Name ($Config.ProcessName -replace '\.exe$', '') -ErrorAction SilentlyContinue
    if (-not $process) {
